...
- 상세에서 장비 4슬롯(무기/투구/갑옷/장신구) 장착/교체/해제 가능
...
## 룬 규칙
- 영웅 룬(`H1..H6`)은 해당 영웅의 역할 행동을 발동합니다.
- 전술 룬:
  - `🤝 협공`: 살아있는 아군 1명이 랜덤 적을 보조 공격
  - `🧱 방비`: 파티 보호막 + 회복
...
- 스핀 표식 시스템(보상 특성으로 확률 강화 가능):
  ...
  - `전술 변환(🧬)`: 일정 확률로 전술 심볼 1개가 생존 영웅 심볼로 변환
...
- 전투 불능 영웅은 룬 풀에서 제외됩니다.
- 제외된 가중치는 전술 룬으로 재분배됩니다.
...
- 룬 조합 시너지:
  ...
  - 전술 공명(전술 3개): 즉시 방비/회복 + 전술 위력 상승
  - 연계 진형(서로 다른 영웅 3개): 협력 보정으로 화력/치명 상승

## 인게임 스킬 고도화 (3회 반영)
- 1차: 영웅 스킬에 상태효과를 추가했습니다.
  - 표식(추가 피해), 화상(턴 피해), 약화(적 공격 감소), 재생(턴 회복), 가드(아군 피해 감소)
- 2차: 보상 특성에 스킬 전용 강화축을 추가했습니다.
  - 스킬 피해/표식 피해/화상 피해/재생 회복/피해 감소 강화
- 3차: 전투 가독성을 위해 유닛 하단에 상태 아이콘(표식/화상/약화/재생/집중)을 표시합니다.
...
- 스핀 특성 밸런스
  - `x2/리롤/과충전/메아리/특수 심볼/확장 릴/선두 복제/전술 변환/룬 각인/연쇄 고리/2번 슬롯 공명/각성 문장` 확률 상승 수치를 과도 중첩되지 않게 조정했습니다.
