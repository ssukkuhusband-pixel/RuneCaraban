...
- 메타 영웅 뽑기(중복 시 조각 획득) + 영웅별 레벨업/진화 성장 시스템
...
- 메인 중앙에 출전 영웅 아이콘 4칸(빈 슬롯 포함) 표시
- 아이콘 탭 시 장착 팝업에서 해당 슬롯 영웅 교체/해제 가능
- 메인 출전 아이콘에서 출전 영웅이 4.5~9초 간격으로 랜덤 대사를 출력
- `영웅 목록`, `영웅 뽑기` 버튼 배치
...
- 영웅 선택 시 상세 정보(레벨/진화/스탯/조각) 표시
- 상세에서 `출전 장착/해제`, `레벨업`, `진화` 가능
...
- 상세 화면에서 영웅별 패시브 3개와 진화 해금 단계 확인 가능
...
- 체력바 아래에 별도 `보호막 게이지`가 표시되어 보호막 잔량을 직관적으로 확인할 수 있습니다.
...
- 영웅 성장:
  - 보유 영웅별 `Lv.1 ~ Lv.10` 레벨업 + `진화 0 ~ 5단계` 2축 성장
  - 레벨업: 비교적 저비용(결정 소모)으로 자주 진행, 기본 스탯이 점진 증가
  - 진화: 동일 영웅 조각 소모, 기본 스탯이 크게 증가
  - 패시브 해금: 진화 단계에 따라 순차 해금(`1/3/5단계`)
...
- 전투 밸런스
  ...
  - 보스 체력을 기존 대비 약 2배로 상향했습니다.
  - 보스 전용 강력 의도(말살 일격/재앙 폭염/응징 반격)를 추가해 단일 고위력, 광역+화상, 반격 패턴을 강화했습니다.
- 보상 밸런스
