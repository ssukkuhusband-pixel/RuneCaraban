...
- 메인 로비에서 출전 영웅 카드가 표시되고, `장착/해제` 및 `◀/▶` 순서 변경이 즉시 반영되는지 확인.
- 메인 로비의 대기 영웅 목록에서 장착 버튼으로 출전 파티에 추가되고 최대 4명 제한이 적용되는지 확인.
- 메인 로비 출전 카드에서 4~8초 주기로 랜덤 대사가 출력되고, 메인 외 화면(영웅 목록/뽑기/전투)에서는 중단되는지 확인.
