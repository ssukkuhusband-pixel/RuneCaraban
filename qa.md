...
- 룬에 부착된 확장 표식(`▲`, `∞`, `⧉`, `🧬`)이 릴 UI에서 확인되고 실제 위력/추가 발동/변환에 반영된다.
...
- 전투 불능 영웅 룬은 제외되고 전술 룬으로 재분배된다.
- 생존 영웅이 1~2명일 때 스핀 결과에 영웅 룬 최소 개수 보정이 반영된다.
- 상단 `연계` 배율이 턴 내 해석 순서에 따라 상승한다.
- 상단 `턴 보너스` pill이 룬 시너지 결과를 올바르게 표시한다.
- 전투 불능 아군 수에 따른 공격 보정이 규칙 문구와 실제 수치에 반영된다.
- 룬 시너지(쌍룬/삼중/전술 공명/연계 진형)가 로그와 전투 수치에 반영된다.
...
- 스킬 전용 특성(스킬 피해/표식 피해/화상 피해/재생/가드 강화)이 보상 풀에서 등장한다.
- 스핀 특화 특성(`x2 표식`, `리롤 표식`, `과충전 표식`, `메아리 표식`, `특수 심볼`, `확장 릴`, `선두 복제`, `전술 변환`)이 등장하고 스핀 결과에 반영된다.
- 보상 3지선다에서 스핀 특성 카드가 과도하게 동시 노출되지 않는지 확인한다.
...
- 신규 특성(흡혈/연사/치명 배율)이 전투 로그와 결과에 반영된다.
...
- 영웅 성장 버튼이 조각/결정 조건을 모두 검사하고 성장 성공 시 레벨/스탯이 다음 런에 반영되는지 확인.
- 영웅 목록 화면의 출전 편성(장착/해제) 변경이 `meta.loadout`에 저장되고 다음 런 시작 파티에 반영되는지 점검.
- 영웅 상세 화면에서 패시브 3개(해금 레벨 포함)가 보이고, 레벨업 시 잠금/해금 상태가 즉시 갱신되는지 확인.
