(() => {
  ...
  const HERO_DIALOG_PROFILE = {
    ...
    H2: {
      ...
      lines: {
        ...
        lobby_idle: [
          "하… 집에 있는 개가 더 보고 싶다.",
          "어젯밤엔 또 잠을 설쳤어. 옛날 꿈만 꾸더라.",
          "{ally}, 네가 옆에 있으니까 그나마 숨이 놓인다.",
          "솔직히 오늘은 그냥 집에 가서 쉬고 싶다.",
          "예전엔 이렇게 화가 많진 않았는데 말이야.",
        ],
    ...
    H3: {
      ...
      lines: {
        ...
        lobby_idle: [
          "이 모닥불 냄새 맡으면 어릴 때 생각나.",
          "{ally}, 너 웃으면 진짜 분위기 살아난다니까!",
          "나중에 다 끝나면 바닷가 가서 쉬자, 응?",
          "오늘만 잘 버티면 따뜻한 밥 먹을 수 있겠지?",
          "집 가면 제일 먼저 침대에 점프할 거야!",
        ],
    ...
    H4: {
      ...
      lines: {
        ...
        lobby_idle: [
          "진형은 내가 볼 테니 마음부터 추슬러.",
          "무리한 진입은 금물이다. 준비가 먼저다.",
          "다들 물과 붕대 챙겨. 기본이 생존이다.",
          "{ally}, 겁나면 숨부터 고르고 내 뒤로 붙어.",
        ],
    ...
    H5: {
      ...
      lines: {
        ...
        lobby_idle: [
          "오늘은 유난히 고향 생각이 나네요.",
          "{ally}, 힘들면 제 어깨에 기대도 괜찮아요.",
          "끝나면 따뜻한 차 한 잔 같이 마셔요.",
          "다들 집에 돌아가면 가장 먼저 누구를 만나고 싶나요?",
          "우리가 서로 다독이면, 이 밤도 지나가요.",
        ],
    ...
    H6: {
      ...
      lines: {
        ...
        lobby_idle: [
          "우리 동네 골목 야경이 가끔 그립다.",
          "집에 가면 아무 말도 안 하고 하루 종일 잘 거야.",
          "{ally}, 네가 옆에 있으면 나도 좀 덜 거칠어진다.",
          "솔직히 오늘은 싸움보다 집밥이 더 끌린다.",
          "예전 친구들 얼굴이 갑자기 떠오르네.",
        ],
  ...
  const HERO_DIALOG_EVENT_FALLBACK = {
    ...
    lobby_idle: ["모닥불 앞에 있으면 마음이 좀 놓인다.", "끝나고 다 같이 쉬는 날이 빨리 오면 좋겠다."],
  ...
  const HERO_DIALOG_TONE_EXTRAS = {
    ...
    angry: {
      ...
      lobby_idle: [
        "하… 아무도 없는 집 생각하면 더 짜증 난다.",
        "{ally}, 네가 멀쩡하면 그걸로 됐다.",
        "끝나면 혼자라도 조용한 데 가서 쉬고 싶다.",
      ],
    ...
    comic: {
      ...
      lobby_idle: [
        "우리 나중에 쉬는 날엔 다 같이 놀러 가자!",
        "{ally}, 오늘 표정 좋다! 그걸로 절반은 이겼어!",
        "전투 끝나면 간식 파티하자, 내가 쏜다!",
      ],
    ...
    calm: {
      ...
      lobby_idle: [
        "마음이 안정되면 전투도 쉬워져요.",
        "{ally}, 오늘도 무사히 돌아가요. 약속해요.",
        "가끔은 아무 일 없는 저녁이 가장 소중하네요.",
      ],
  ...
  const HERO_DIALOG_PERSONA_EXTRAS = {
    H2: {
      ...
      lobby_idle: ["{ally}, 네가 다치면 나 진짜 폭발한다.", "예전 동료들 이름이 아직도 꿈에 나와."],
      attack: ["거슬리면 전부 베어버린다!"],
      low_hp: ["죽기 전까지 화내면서 버틴다."],
    },
    H3: {
      battle_start: ["좋아! 오늘도 해피엔딩으로 간다!"],
      lobby_idle: ["{ally}, 끝나면 우리 동네 국수집 데려갈게!", "집 가서 푹 자는 상상만 해도 기분 좋아!"],
      attack: ["위험해도 괜찮아~ 내가 웃으면서 커버할게!"],
      low_hp: ["체력은 낮아도 희망은 높아!"],
    },
    H6: {
      battle_start: ["오늘은 진짜 다 박살낸다."],
      lobby_idle: ["{ally}, 네가 옆에 있으면 나도 좀 사람 같아진다.", "솔직히 말하면, 나도 집이 그립다."],
      attack: ["막지 마. 전부 관통한다!"],
  ...
  function formatDialogLine(template, context = {}) {
    ...
    return template
      ...
      .replaceAll("{ally}", context.ally || "동료")
  ...
  function pickLobbyTalkLine(hero) {
    ...
    const seriousLobbyHeroIds = new Set(["H1", "H4"]);
    const isSeriousLobby = seriousLobbyHeroIds.has(hero.id);
    const eventPool = [];
    const lobbyLines = mergedDialogLines("hero", hero, profile, "lobby_idle");
    if (lobbyLines.length > 0) eventPool.push("lobby_idle", "lobby_idle", "lobby_idle", "lobby_idle");
    if (isSeriousLobby && mergedDialogLines("hero", hero, profile, "idle_wait").length > 0) eventPool.push("idle_wait");
    ...
    const allyPool = currentLoadout()
      .map((heroId) => heroById(heroId))
      .filter((entry) => entry && entry.id !== hero.id);
    const ally = allyPool.length > 0 ? allyPool[randInt(allyPool.length)].name : "다들";

    return formatDialogLine(line, { self: hero.name || "", ally });
  }
